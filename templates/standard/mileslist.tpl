						...
						<div class="marker_all">
							<div id="miles_{$milestones[stone].ID}" class="focus_off">
								...
								<table class="line" cellpadding="0" cellspacing="0">
									<tr>
										<td class="a">
											<a class="butn_check" href="managemilestone.php?action=close&amp;mid={$milestones[stone].ID}&amp;id={$project.ID}" title="{#close#}"></a>
										</td>
										<td class="b"></td>
										<td class="c">{$milestones[stone].fend}</td>
										<td class="days">{$milestones[stone].daysleft}</td>
										<td class="tools">
											<a class="tool_edit" href="managemilestone.php?action=editform&amp;mid={$milestones[stone].ID}&amp;id={$project.ID}"></a><a class="tool_del" href="managemilestone.php?action=del&amp;mid={$milestones[stone].ID}&amp;id={$project.ID}" title="{#delete#}"></a>
										</td>
									</tr>
								</table>
						
								<div class="link_in_toggle">
									<a href="managemilestone.php?action=showmilestone&amp;msid={$milestones[stone].ID}&amp;id={$project.ID}">
										{$milestones[stone].name|truncate:30:"...":true}
									</a>
								</div>
											...
											<td class="descript">
												...
												{section name=task loop=$milestones[stone].tasks}
													{if $smarty.section.task.first == TRUE}
														...
														<b>{#tasklists#}:</b>
													{/if}
													<a href="managetasklist.php?action=showtasklist&amp;tlid={$milestones[stone].tasks[task].ID}&amp;id={$project.ID}">
														{$milestones[stone].tasks[task].name}
													</a>
												...
												{section name=msg loop=$milestones[stone].messages}
													{if $smarty.section.msg.first == TRUE}
														...
														<b>{#messages#}:</b>
													{/if}
													<a href="managemessage.php?action=showmessage&amp;mid={$milestones[stone].messages[msg].ID}&amp;id={$project.ID}">
														{$milestones[stone].messages[msg].title}
													</a>
