...
<div id="content-left">
								...
								{section name=task loop=$lists[list].tasks}
									...
									{/if}
										<tr {if $lists[list].tasks[task].daysleft < 0} class="marker-late"{elseif $lists[list].tasks[task].daysleft == 0} class="marker-today"{/if}>
											...
											<td>
												<div class="toggle-in">
													<span class="acc-toggle" onclick="javascript:accord_{$lists[list].ID}.activate($$('#acc_{$lists[list].ID} .accordion_toggle')[{$smarty.section.task.index}]);toggleAccordeon('acc_{$lists[list].ID}',this);">
													</span>
											...
											<td style="text-align:right">{$lists[list].tasks[task].daysleft}&nbsp;&nbsp;
											</td>
											<td class="tools">
												{if $userpermissions.tasks.edit}
										...
										{section name=oldtask loop=$lists[list].oldtasks}
											...
											{/if}
												<tr>
													...
													<td class="b">
														<div class="toggle-in">
															<span class="acc-toggle" onclick="javascript:accord_done_{$lists[list].ID}.activate($$('#done_{$lists[list].ID} .accordion_toggle')[{$smarty.section.oldtask.index}]);toggleAccordeon('done_{$lists[list].ID}',this);">
															</span>
													...
													<td class="c">
														<a href="manageuser.php?action=profile&amp;id={$lists[list].oldtasks[oldtask].user_id}">{$lists[list].oldtasks[oldtask].user|truncate:23:"...":true}</a>
													</td>
	...
	{if $oldlists[0][0]} {*only show the block if there are closed tasklists*} {*Done Tasklists*}
		...
		<div id="content-left-in"> {*closed tasklists*}
			<div class="tasks">
							...
							{section name = oldlist loop=$oldlists}
								...
								{/if}
									<tr {if $oldlists[oldlist].daysleft < 0} class="marker-late"{elseif $oldlists[oldlist].daysleft == 0} class="marker-today"{/if}>
										<td>
											{if $userpermissions.tasks.close}
												<a class="butn_check" href="managetasklist.php?action=open&amp;tlid={$oldlists[oldlist].ID}&amp;id={$project.ID}" title="{#open#}"></a>
											{/if}
										</td>
										<td>
											<div class="toggle-in">
												<span class="acc-toggle" onclick="javascript:accord_donelists.activate($$('#block-donelists .accordion_toggle')[{$smarty.section.oldlist.index}]);toggleAccordeon('acc_{$lists[list].ID}',this);">
												</span>
