 ...
 * Version string.
 */
define("FEEDCREATOR_VERSION", "Collabtive");
...
class HTMLCreator extends FeedCreator {
    ...
    function createFeed()
    {
        ...
        $feedArray = array();
        if ($this->image != null) {
            $imageStr = "<a href='" . $this->image->link . "'" . $targetInsert . ">" . "<img src='" . $this->image->url . "' border='0' alt='" .
            FeedCreator::iTrunc(htmlspecialchars($this->image->title), 100) . "' align='" . $this->imageAlign . "' ";
            if ($this->image->width) {
        ...
        if ($this->getDescription()) {
            ...
            str_replace("]]>", "", str_replace("<![CDATA[", "", $this->getDescription())) . "</div>";
        }
        ...
        for ($i = 0;$i < count($this->items);$i++) {
            ...
            if ($this->items[$i]->title) {
                if ($this->items[$i]->link) {
                    ...
                    FeedCreator::iTrunc(htmlspecialchars(strip_tags($this->items[$i]->title)), 100) . "</div>";
                }
            }
            if ($this->items[$i]->getDescription()) {
                $feedArray[] = "<div class='" . $this->stylePrefix . "item_description'>" .
                str_replace("]]>", "", str_replace("<![CDATA[", "", $this->items[$i]->getDescription())) . "</div>";
            }
        }
        if ($this->footer) {
