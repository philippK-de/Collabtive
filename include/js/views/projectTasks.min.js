...
function initTasklistViews(){var a=new taskFormHandler,c=document.getElementsByClassName("blockaccordion_content");projectTaskViews=[];for(var b=0;b<c.length;b++){var f=createView({el:c[b].id,itemType:"task",url:"managetask.php?action=projectTasks&tlid="+c[b].dataset.tasklist+"&id="+c[b].dataset.project,dependencies:[]});projectTaskViews.push(f)}accordeons=[];projectTaskViews[projectTaskViews.length-1].afterUpdate(function(){for(var a=document.getElementsByClassName("closeElement"),b=0;b<a.length;b++)a[b].onclick=
function(a){handleClose(a)};a=document.getElementsByClassName("deleteElement");for(b=0;b<a.length;b++)a[b].onclick=function(a){handleDelete(a)};a=cssAll(".taskList");for(b=0;b<a.length;b++)accordeons.push(new accordion2(a[b].id))});a.forms=document.getElementsByClassName("taskSubmitForm");a.views=projectTaskViews;a.bindViews()}initTasklistViews();
