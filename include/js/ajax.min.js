...
function confirmit(a,b){check=confirm(a);b=decodeURI(b);1==check&&(window.location=b)}function confirmfunction(a,b){1==confirm(a)&&eval(b)}function setCookie(a,b,c,d,e,f){var g=new Date;g.setTime(g.getTime());c&&(c*=864E5);g=new Date(g.getTime()+c);document.cookie=a+"="+escape(b)+(c?";expires="+g.toGMTString():"")+(d?";path="+d:"")+(e?";domain="+e:"")+(f?";secure":"")}
function readCookie(a){a+="=";for(var b=document.cookie.split(";"),c=0;c<b.length;c++){for(var d=b[c];" "==d.charAt(0);)d=d.substring(1,d.length);if(0==d.indexOf(a))return d.substring(a.length,d.length)}return null}function getnow(a){var b=new Date,c=b.getHours(),b=b.getMinutes();10>c&&(c="0"+c);10>b&&(b="0"+b);$(a).value=c+":"+b}
function calctime(){var a=new Date,b=a.getHours(),c=a.getMinutes(),a=a.getSeconds();10>b&&(b="0"+b);10>c&&(c="0"+c);10>a&&(a="0"+a);$("digitalclock").innerHTML=b+":"+c+":"+a;setTimeout("calctime()",1E3)}function systemMsg(a){new Effect.Appear(a,{duration:2});window.setTimeout("new Effect.Fade('"+a+"', { duration: 2.0 })",5E3)}function addEngine(a){window.external.AddSearchProvider(a)}function sortit(){}
function sortBlock(a,b){for(var c=document.getElementById(a),d=c.getElementsByTagName("tbody"),e=[],f=0;f<d.length;f++)e.push(d[f]);for(d=0;d<e.length;d++)f=e[d].getAttribute("rel").split(","),e[d].setAttribute("theid",Number(f[0])),e[d].setAttribute("title",f[1]),e[d].setAttribute("daysleft",f[2]),e[d].setAttribute("project",f[3]),e[d].setAttribute("theuser",f[4]),e[d].setAttribute("done",f[5]),"asc"==e[d].getAttribute("sortorder")?e[d].setAttribute("sortorder","desc"):e[d].setAttribute("sortorder",
"asc"),c.removeChild(e[d]);"daysleft"==b?e.sort(daysort):"done"==b?e.sort(done):"project"==b?e.sort(sortByProject):"byuser"==b?e.sort(sortByUser):e.sort(sortByTitle);for(d=0;d<e.length;d++)c.appendChild(e[d]),f=e[d].getAttribute("theid"),document.getElementById(a+"toggle"+f).setAttribute("onclick","javascript:accord_tasks.activate(document.querySelector('#taskhead_content"+d+"'));")}
