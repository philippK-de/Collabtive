...
onFailure:function(){console.log("error")}})}}function startWait(a){$(a).style.display="block"}function stopWait(a){$(a).style.display="none"}function deleteElement(a,b,c){new Ajax.Request(b,{method:"get",onSuccess:function(b){"ok"==b.responseText&&(removeRow(a,deleteEndcolor),void 0!=c&&updateView(c))}});try{systemMsg("deleted")}catch(d){}}
function closeElement(a,b,c){new Ajax.Request(b,{method:"get",onSuccess:function(b){"ok"==b.responseText&&(removeRow(a,closeEndcolor),void 0!=c&&updateView(c))}});try{systemMsg("closed")}catch(d){}}function recolorRows(a){a=a.options.rowid;try{var b=$(a).parentNode,c=[];$$("#"+b.id+" tbody:not([id="+a+"])").each(function(a){"none"!=a.style.display&&c.push(a)});for(b=0;b<c.length;b++)0==b%2?$(c[b].id).className="color-a":$(c[b].id).className="color-b"}catch(d){}}
