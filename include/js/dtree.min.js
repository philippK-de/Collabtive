function Node(id,pid,name,url,title,target,icon,iconOpen,open,daysLeft){this.id=id;this.pid=pid;this.name=name;this.url=url;this.title=title;this.target=target;this.icon=icon;this.daysLeft=daysLeft;this.iconOpen=iconOpen;this._io=open||false;this._is=false;this._ls=false;this._hc=false;this._ai=0;this._p}
...
line:"templates/standard/theme/standard/images/symbols/line.gif",join:"templates/standard/theme/standard/images/symbols/join.gif",joinBottom:"templates/standard/theme/standard/images/symbols/joinbottom.gif",plus:"templates/standard/theme/standard/images/symbols/plus.gif",plusBottom:"templates/standard/theme/standard/images/symbols/plusbottom.gif",minus:"templates/standard/theme/standard/images/symbols/minus.gif",minusBottom:"templates/standard/theme/standard/images/symbols/minusbottom.gif",nlPlus:"templates/standard/theme/standard/images/symbols/nolines_plus.gif",
nlMinus:"templates/standard/theme/standard/images/symbols/nolines_minus.gif"};this.obj=objName;this.aNodes=[];this.aIndent=[];this.root=new Node(-1);this.selectedNode=null;this.selectedFound=false;this.completed=false}dTree.prototype.add=function(id,pid,name,url,title,target,icon,iconOpen,open,daysLeft){this.aNodes[this.aNodes.length]=new Node(id,pid,name,url,title,target,icon,iconOpen,open,daysLeft)};dTree.prototype.openAll=function(){this.oAll(true)};dTree.prototype.closeAll=function(){this.oAll(false)};
...
dTree.prototype.addNode=function(pNode){var str="";var n=0;if(this.config.inOrder)n=pNode._ai;for(n;n<this.aNodes.length;n++)if(this.aNodes[n].pid==pNode.id){var cn=this.aNodes[n];cn._p=pNode;cn._ai=n;this.setCS(cn);if(!cn.target&&this.config.target)cn.target=this.config.target;if(cn._hc&&!cn._io&&this.config.useCookies)cn._io=this.isOpen(cn.id);if(!this.config.folderLinks&&cn._hc)cn.url=null;if(this.config.useSelection&&cn.id==this.selectedNode&&!this.selectedFound){cn._is=true;this.selectedNode=
n;this.selectedFound=true}str+=this.node(cn,n);if(cn._ls)break}return str};
dTree.prototype.node=function(node,nodeId){var str;str='<div class="dTreeNode">'+this.indent(node,nodeId);if(this.config.useIcons){if(!node.icon)node.icon=this.root.id==node.pid?this.icon.root:node._hc?this.icon.folder:this.icon.node;if(!node.iconOpen)node.iconOpen=node._hc?this.icon.folderOpen:this.icon.node;if(this.root.id==node.pid){node.icon=this.icon.root;node.iconOpen=this.icon.root}if(this.root.id!=node.pid)str+='<img id="i'+this.obj+nodeId+'" src="'+(node._io?node.iconOpen:node.icon)+'" alt="" style = "height:27px;width:27px;" />'}if(node.url){str+=
'<a id="s'+this.obj+nodeId+'" class="'+(this.config.useSelection?node._is?"nodeSel":"node":"node")+'" href="'+node.url+'"';if(node.title)str+=' title="'+node.title+'"';if(node.target)str+=' target="'+node.target+'"';if(this.config.useStatusText)str+=" onmouseover=\"window.status='"+node.name+"';return true;\" onmouseout=\"window.status='';return true;\" ";if(this.config.useSelection&&(node._hc&&this.config.folderLinks||!node._hc))str+=' onclick="javascript: '+this.obj+".s("+nodeId+');"';if(typeof node.daysLeft==
"number")if(node.daysLeft<0)str+=' style = "color:#be4c43 "';str+=">"}else if((!this.config.folderLinks||!node.url)&&node._hc&&node.pid!=this.root.id)str+='<a href="javascript: '+this.obj+".o("+nodeId+');" class="node">';str+=node.name;if(node.url||(!this.config.folderLinks||!node.url)&&node._hc)str+="</a>";str+="</div>";if(node._hc){str+='<div id="d'+this.obj+nodeId+'" class="clip" style="display:'+(this.root.id==node.pid||node._io?"block":"none")+';">';str+=this.addNode(node);str+="</div>"}this.aIndent.pop();
return str};
dTree.prototype.indent=function(node,nodeId){var str="";if(this.root.id!=node.pid){for(var n=0;n<this.aIndent.length;n++)str+='<img src="'+(this.aIndent[n]==1&&this.config.useLines?this.icon.line:this.icon.empty)+'" alt="" style = "height:26px;width:26px;" />';node._ls?this.aIndent.push(0):this.aIndent.push(1);if(node._hc){str+='<a href="javascript: '+this.obj+".o("+nodeId+');"><img id="j'+this.obj+nodeId+'" src="';if(!this.config.useLines)str+=node._io?this.icon.nlMinus:this.icon.nlPlus;else str+=
node._io?node._ls&&this.config.useLines?this.icon.minusBottom:this.icon.minus:node._ls&&this.config.useLines?this.icon.plusBottom:this.icon.plus;str+='" alt="" style = "height:26px;width:26px;"  /></a>'}else str+='<img src="'+(this.config.useLines?node._ls?this.icon.joinBottom:this.icon.join:this.icon.empty)+'" alt="" style = "height:26px;width:26px;" />'}return str};
dTree.prototype.setCS=function(node){var lastId;for(var n=0;n<this.aNodes.length;n++){if(this.aNodes[n].pid==node.id)node._hc=true;if(this.aNodes[n].pid==node.pid)lastId=this.aNodes[n].id}if(lastId==node.id)node._ls=true};dTree.prototype.getSelected=function(){var sn=this.getCookie("cs"+this.obj);return sn?sn:null};
...
dTree.prototype.o=function(id){var cn=this.aNodes[id];this.nodeStatus(!cn._io,id,cn._ls);cn._io=!cn._io;if(this.config.closeSameLevel)this.closeLevel(cn);if(this.config.useCookies)this.updateCookie()};dTree.prototype.oAll=function(status){for(var n=0;n<this.aNodes.length;n++)if(this.aNodes[n]._hc&&this.aNodes[n].pid!=this.root.id){this.nodeStatus(status,n,this.aNodes[n]._ls);this.aNodes[n]._io=status}if(this.config.useCookies)this.updateCookie()};
...
dTree.prototype.clearCookie=function(){var now=new Date;var yesterday=new Date(now.getTime()-1E3*60*60*24);this.setCookie("co"+this.obj,"cookieValue",yesterday);this.setCookie("cs"+this.obj,"cookieValue",yesterday)};dTree.prototype.setCookie=function(cookieName,cookieValue,expires,path,domain,secure){document.cookie=escape(cookieName)+"="+escape(cookieValue)+(expires?"; expires="+expires.toGMTString():"")+(path?"; path="+path:"")+(domain?"; domain="+domain:"")+(secure?"; secure":"")};
...
dTree.prototype.updateCookie=function(){var str="";for(var n=0;n<this.aNodes.length;n++)if(this.aNodes[n]._io&&this.aNodes[n].pid!=this.root.id){if(str)str+=".";str+=this.aNodes[n].id}this.setCookie("co"+this.obj,str)};dTree.prototype.isOpen=function(id){var aOpen=this.getCookie("co"+this.obj).split(".");for(var n=0;n<aOpen.length;n++)if(aOpen[n]==id)return true;return false};
if(!Array.prototype.push)Array.prototype.push=function array_push(){for(var i=0;i<arguments.length;i++)this[this.length]=arguments[i];return this.length};if(!Array.prototype.pop)Array.prototype.pop=function array_pop(){lastElement=this[this.length-1];this.length=Math.max(this.length-1,0);return lastElement};
