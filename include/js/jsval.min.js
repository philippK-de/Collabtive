...
function _checkRegExp(a,b){switch(a){case "JSVAL_RX_EMAIL":return/^[a-zA-Z\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd]+([\.-]?[a-zA-Z\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd]+)*@[a-zA-Z\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd]+([\.-]?[a-zA-Z\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd\ufffd]+)*(\.\w{2,5})+$/.test(b);case "JSVAL_RX_TEL":return/^1?[\- ]?\(?\d{3}\)?[\- ]?\d{3}[\- ]?\d{4}$/.test(b);case "JSVAL_RX_PC":return/^[a-z]\d[a-z] ?\d[a-z]\d$/i.test(b);case "JSVAL_RX_ZIP":return/^\d{5}$/.test(b);case "JSVAL_RX_MONEY":return/^\d+([\.]\d\d)?$/.test(b);
case "JSVAL_RX_CREDITCARD":return!isNaN(b);case "JSVAL_RX_POSTALZIP":if(6==b.length||7==b.length)return/^[a-zA-Z]\d[a-zA-Z] ?\d[a-zA-Z]\d$/.test(b);if(5==b.length||10==b.length)return/^\d{5}(\-\d{4})?$/.test(b);break;default:return a.test(b)}};
