Vue.config.silent=!0;
function createView(a){var b={items:[],pages:[],limit:pagination.itemsPerPage,offset:0,currentPage:1,itemsCount:0,itemType:a.itemType,dependencies:a.dependencies,url:a.url};new Ajax.Request(b.url,{method:"get",onSuccess:function(a){console.log("url "+b.url);a=JSON.parse(a.responseText);b.items=a.items;b.itemsCount=a.count;a=pagination.listPages(a.count);b.pages=a;c.$emit("iloaded")},onLoading:function(){startWait("progress"+a.el)},onComplete:function(){stopWait("progress"+a.el)},onFailure:function(){}});
var c=new Vue({el:"#"+a.el,data:b});return c}
function updateView(a,b){void 0===b&&(b=!0);var c=a.url;0<a.limit&&(c+="&limit="+a.limit);0<a.offset&&(c+="&offset="+a.offset);console.log(c);new Ajax.Request(c,{method:"get",onSuccess:function(c){c=JSON.parse(c.responseText);Vue.set(a,"items",c.items);Vue.set(a,"pages",pagination.listPages(c.count));if(1==b&&(c=a.$get("dependencies"),0<c.length))for(var e=0;e<c.length;e++)Vue.set(c[e],"limit",a.limit),Vue.set(c[e],"offset",a.offset),updateView(c[e],c[e].url)},onLoading:function(){startWait("progress"+
a.$el.id)},onComplete:function(){stopWait("progress"+a.$el.id);a.$emit("updated")},onFailure:function(){}})}
...
function deleteElement(a,b,c){new Ajax.Request(b,{method:"get",onSuccess:function(b){"ok"==b.responseText&&(removeRow(a,deleteEndcolor),void 0!=c&&(updateView(c),systemMessage.deleted(c.$get("itemType"))))}})}function closeElement(a,b,c){new Ajax.Request(b,{method:"get",onSuccess:function(b){if("ok"==b.responseText){try{removeRow(a,closeEndcolor)}catch(e){}void 0!=c&&(updateView(c),systemMessage.closed(c.$get("itemType")))}}})}
...
Vue.component("pagination",paginationComponent);var progressComponent=Vue.extend({props:["block","loader"],template:'<div class="progress" id="progress{{block}}" style="float:left;display:none;">    <img src="templates/standard/theme/standard/images/symbols/{{loader}}"/> </div>'});Vue.component("loader",progressComponent);Vue.filter("truncate",function(a,b){return a.substr(0,b)+"..."});
