Vue.config.silent=!0;
function createView(a){var b={items:[],pages:[],limit:10,offset:0,currentPage:1,itemsCount:0,itemType:a.itemType,dependencies:a.dependencies,url:a.url};new Ajax.Request(b.url,{method:"get",onSuccess:function(a){console.log("url "+b.url);a=JSON.parse(a.responseText);b.items=a.items;b.itemsCount=a.count;a=pagination.listPages(a.count);b.pages=a;c.$emit("iloaded")},onLoading:function(){startWait("progress"+a.el)},onComplete:function(){stopWait("progress"+a.el)},onFailure:function(){}});var c=new Vue({el:"#"+
a.el,data:b});return c}
