...
Object.extend(Control.Modal, {
    ...
    current: false,
    ie: false,
    effects: {containerFade: false, containerAppear: false, overlayFade: false, overlayAppear: false},
    targetRegexp: /#(.+)$/,
    imgRegexp: /\.(jpe?g|gif|png|tiff?)$/,
    overlayStyles: {position: "fixed", top: 0, left: 0, width: "100%", height: "100%", zIndex: 9998},
    overlayIEStyles: {position: "absolute", top: 0, left: 0, zIndex: 9998},
    ...
    load: function () {
        if (!Control.Modal.loaded) {
            ...
            _1.appendChild(Control.Modal.overlay);
            _1.appendChild(Control.Modal.container);
            _1.appendChild(Control.Modal.loading);
            Control.Modal.container.observe("mouseout", function (_2) {
                if (!Control.Modal.disableHoverClose && Control.Modal.current && Control.Modal.current.options.hover && !Position.within(Control.Modal.container, Event.pointerX(_2), Event.pointerY(_2))) {
                    Control.Modal.close();
                }
            });
        }
    ...
    open: function (_3, _4) {
        ...
        var _5 = new Control.Modal(false, _4);
        _5.open();
        return _5;
    },
    close: function (_6) {
        ...
        if (Control.Modal.current) {
            Control.Modal.current.close(_6);
        }
    },
    ...
    center: function (_7) {
        if (!_7._absolutized) {
            _7.setStyle({position: "absolute"});
            _7._absolutized = true;
        }
        var _8 = _7.getDimensions();
        Position.prepare();
        var _9 = (Position.deltaX + Math.floor((Control.Modal.getWindowWidth() - _8.width) / 2));
        var _a = (Position.deltaY + ((Control.Modal.getWindowHeight() > _8.height) ? Math.floor((Control.Modal.getWindowHeight() - _8.height) / 2) : 0));
        _7.setStyle({
            top: ((_8.height <= Control.Modal.getDocumentHeight()) ? ((_a != null && _a > 0) ? _a : "0") + "px" : 0),
            left: ((_8.width <= Control.Modal.getDocumentWidth()) ? ((_9 != null && _9 > 0) ? _9 : "0") + "px" : 0)
        });
    },
...
Object.extend(Control.Modal.prototype, {
    ...
    initialize: function (_c, _d) {
        ...
        Object.extend(this.options, _d || {});
        var _e = false;
        var _f = false;
        if (this.element) {
            _e = Control.Modal.targetRegexp.exec(this.element.href);
            _f = Control.Modal.imgRegexp.exec(this.element.href);
        }
        ...
        if (this.element) {
            if (this.options.hover) {
                ...
                this.element.observe("mouseout", function (_11) {
                    if (!Position.within(Control.Modal.container, Event.pointerX(_11), Event.pointerY(_11))) {
                        this.close();
                    }
                }.bindAsEventListener(this));
            } else {
                this.element.onclick = function (_12) {
                    this.open();
                    Event.stop(_12);
                    return false;
                }.bindAsEventListener(this);
        ...
        var _13 = Control.Modal.targetRegexp.exec(window.location);
        this.position = function (_14) {
            if (this.options.position == "absolute") {
                Control.Modal.center(Control.Modal.container);
            } else {
                var xy = (_14 && this.options.position == "mouse" ? [Event.pointerX(_14), Event.pointerY(_14)] : Position.cumulativeOffset(this.element));
            ...
            if (Control.Modal.ie) {
                Control.Modal.overlay.setStyle({height: Control.Modal.getDocumentHeight() + "px", width: Control.Modal.getDocumentWidth() + "px"});
            }
        }.bind(this);
        if (this.mode == "named" && this.options.autoOpenIfLinked && _13 && _13[1] && _13[1] == this.href) {
            this.open();
        }
    },
    ...
    open: function (_18) {
        if (!_18 && this.notify("beforeOpen") === false) {
            return;
        }
        if (!Control.Modal.loaded) {
            Control.Modal.load();
        ...
        if (!this.options.hover) {
            Control.Modal.overlay.setStyle({zIndex: this.options.zIndex, opacity: this.options.opacity});
        }
        Control.Modal.container.setStyle({
        ...
        if (Control.Modal.ie && !this.options.hover) {
            $A(document.getElementsByTagName("select")).each(function (_19) {
        ...
        switch (this.mode) {
            ...
            case "ajax":
                ...
                var _1c = {
                    method: "post", onSuccess: function (_1d) {
                        this.hideLoadingIndicator();
                        this.update(_1d.responseText);
                        this.notify("onSuccess", _1d);
                ...
                Object.extend(_1c, this.options.requestOptions);
                this.showLoadingIndicator();
                this.ajaxRequest = new Ajax.Request(this.href, _1c);
                break;
            case "iframe":
                ...
                if (this.options.fade) {
                    ...
                    Control.Modal.effects.overlayAppear = new Effect.Appear(Control.Modal.overlay, {
                        queue: {
                            position: "front",
                            scope: "Control.Modal"
                        }, to: this.options.opacity, duration: this.options.fadeDuration / 2
                    });
                } else {
    ...
    update: function (_1e) {
        if (typeof (_1e) == "string") {
            Control.Modal.container.update(_1e);
        } else {
            Control.Modal.container.update("");
            (_1e.each) ? _1e.each(function (_1f) {
                Control.Modal.container.appendChild(_1f);
        ...
        if (this.options.fade) {
            ...
            Control.Modal.effects.containerAppear = new Effect.Appear(Control.Modal.container, {
                queue: {position: "end", scope: "Control.Modal"},
                to: 1,
    ...
    close: function (_20) {
        if (!_20 && this.notify("beforeClose") === false) {
            return;
        }
        if (this.ajaxRequest) {
        ...
        if (this.mode == "image") {
            var _21 = $("modal_image");
            if (this.options.imageCloseOnClick && _21) {
                _21.stopObserving("click", Control.Modal.close);
            }
        }
        if (Control.Modal.ie && !this.options.hover) {
            $A(document.getElementsByTagName("select")).each(function (_22) {
        ...
        if (this.options.fade) {
            ...
            Control.Modal.effects.containerFade = new Effect.Fade(Control.Modal.container, {
                queue: {position: "front", scope: "Control.Modal"},
                from: 1,
                to: 0,
                duration: this.options.fadeDuration / 2,
                afterFinish: function () {
    ...
    resetClassNameAndStyles: function () {
        ...
        Control.Modal.container.setStyle({height: null, width: null, top: null, left: null});
    },
    notify: function (_23) {
        try {
            if (this.options[_23]) {
                return [this.options[_23].apply(this.options[_23], $A(arguments).slice(1))];
            }
        } catch (e) {
            if (e != $break) {
                throw e;
