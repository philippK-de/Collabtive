...
calendar.prototype.getDatepicker=function(f){if("m/d/Y"==this.dateFormat||"m/d/y"==this.dateFormat)this.dateSeparator="/";var a="",e=this.month,b=this.month+2;this.theDiv=f;this.relateTo&&(document.getElementById(this.relateTo).onfocus=function(){new Effect.Appear(f,{duration:.8})});a=a+'<table class="cal" cellpadding="0" cellspacing="1" border = "0">'+('<tr class="head" ><td class = "back"><a href = "javascript:void(0);"></a></td><td colspan="5" >'+this.monthNames[this.month]+" "+this.year+'</td><td class="next"><a href = "javascript:void(0);"></a></td></tr>');
a+='<tr class = "weekday">';for(i=0;i<this.dayNames.length;i++)a+="<td >"+this.dayNames[i]+"</td>";var a=a+"</tr>",g=this.buildCal();if(!this.keepEmpty&&!document.getElementById(this.relateTo).value){var c,d;c=10>this.month+1?"0"+(this.month+1):this.month+1;d=10>this.currday&&0<this.currday?"0"+this.currday:this.currday;var h=strtotime(c+"/"+d+"/"+this.curryear);document.getElementById(this.relateTo).value=date(this.dateFormat,h)}h=document.getElementById(this.relateTo).value.split(this.dateSeparator);
for(j=0;j<this.weeksInMonth;j++){a+="<tr>";for(i=0;7>i;i++){var k=g[j][i];d=k;10>k&&0<k&&(d="0"+k);c=10>this.month+1?"0"+(this.month+1):this.month+1;c=strtotime(c+"/"+d+"/"+this.year);c=date(this.dateFormat,c);0<k&&k<=this.daysInMonth?a=this.currmonth==this.month&&this.curryear==this.year&&this.currday==k?a+('<td class = "today" onclick = "$(\''+this.relateTo+"').value='"+c+"';new Effect.Fade('"+f+"','{duration:0.6}');\">"+k+"</td>"):this.month==h[1]-1&&this.year==h[2]&&h[0]==k?a+('<td class = "red" onclick = "$(\''+
this.relateTo+"').value='"+c+"';new Effect.Fade('"+f+"','{duration:0.6}');\">"+k+"</td>"):a+('<td class = "normalday" onclick = "$(\''+this.relateTo+"').value='"+c+"';new Effect.Fade('"+f+"','{duration:0.6}');\">"+k+"</td>"):1>k?a+='<td class = "wrong">'+(this.daysLastMonth+k)+"</td>":k>this.daysInMonth&&(a+='<td class = "wrong">'+(k-this.daysInMonth)+"</td>")}a+="</tr>"}a+='<tr><td colspan = "7" class = "dpfoot"><a href = "javascript:void(0);" onclick = "javascript:new Effect.Fade(\''+f+"','{duration:0.6}');\">Close</a></td></tr></table>";
document.getElementById(f).innerHTML=a;var l=this.monthNames,m=this.dayNames,n=this.keepEmpty,p=this.dateSeparator,q=this.year,r=this.relateTo,t=this.dateFormat;document.querySelector("#"+f+" .cal .back a").addEventListener("click",function(){var a=new calendar(e,q);a.monthNames=l;a.dayNames=m;a.keepEmpty=n;a.relateTo=r;a.dateSeparator=p;a.dateFormat=t;a.getDatepicker(f)});document.querySelector("#"+f+" .cal .next a").addEventListener("click",function(){var a=new calendar(b,q);a.monthNames=l;a.dayNames=
m;a.keepEmpty=n;a.relateTo=r;a.dateSeparator=p;a.dateFormat=t;a.getDatepicker(f)})};calendar.prototype.showDatepicker=function(){};calendar.prototype.buildCal=function(){var f=0;for(j=0;j<this.weeksInMonth;j++)for(this.calendar[j]=[],i=0;7>i;i++)f++,this.calendar[j][i]=f-this.startDay;return this.calendar};calendar.prototype.getDaysInMonth=function(f,a){for(var e=new Date(a,f),b=28,g=!1;!g;)e.setDate(b+1),e.getMonth()!=f?g=!0:b++;return b};
