...
define( 'ICALCREATOR_VERSION', 'iCalcreator 2.2.6' );
/*********************************************************************************/
/*********************************************************************************/
/**
 * vcalendar class
 ...
 */
  function getComponent( $arg1=FALSE, $arg2=FALSE ) {
    ...
    $index  -= 1;
    if( !empty( $index) && ( $index > end( array_keys( $this->components ))))
      return FALSE;
    $cix1gC = 0;
    foreach ( $this->components as $cix => $component) {
      unset( $component->propix );
 ...
 */
  function _cmpfcn( $a, $b ) {
    $c = ( !empty( $a->dtend['value'] )) ? $a->dtend['value'] : null;
    if(     empty( $c ) && !empty( $a->due['value'] ))
      $c = $a->due['value'];
    if(     empty( $c ) && !empty( $a->duration['value'] ))
      $c = $a->duration2date();
    $d = ( !empty( $b->dtend['value'] )) ? $b->dtend['value'] : null;
 ...
 */
  function parse( $filename=FALSE ) {
    ...
    if( !filesize( $filename ))
      return FALSE;                 /* err 3 */
    $rows    = file( $filename );
            /* identify and remove BEGIN:VCALENDAR row */
    if( 'BEGIN:VCALENDAR' != strtoupper( trim( $rows[0] )))
      ...
      $rows  = array_values( $rows );
            ...
            /* identify and remove END:VCALENDAR row */
  ...
  //  echo 'setRdate i2 '; print_r ( $rPeriod ); echo "<br />\n"; // test ##
            ...
            elseif( 8 <= strlen( trim( $rPeriod ))) { // ex. 2006-08-03 10:12:18
              ...
              $inputab['tz'] = ( isset( $input['params']['TZID'] ))
                             ? (string) $input['params']['TZID'] :
                               ( isset( $inputab['tz'] )) ? (string) $inputab['tz'] : null;
              $inputa[]      = $inputab;
              $parno         = ( !isset( $parno )) ? count( $inputab ) : $parno;
            }
          }
 ...
 */
  function _date_time_string( $datetime, $parno=FALSE ) {
    ...
    elseif( ctype_digit( substr( $datetime, 0, 8 )) &&
      ...
      $datetime = substr( $datetime, 0, 8 ).substr( $datetime, 9 );
 ...
 * @return array
 */
  function duration2date() {
    if( !isset( $this->dtstart['value'] ) || !isset( $this->duration['value'] ))
      return null;
