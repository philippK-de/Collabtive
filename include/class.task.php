...
class task
{
    ...
    function getMyTodayProjectTasks($project, $limit = 10)
    {
        ...
        $projectTasksStmt = $conn->prepare("SELECT tasks.*,tasks_assigned.user FROM tasks,tasks_assigned WHERE tasks.ID = tasks_assigned.task AND tasks_assigned.user = ? AND tasks.project = ?  AND status=1 AND end = '$now' ORDER BY `end` ASC LIMIT $limit");
        $projectTasksStmt->execute(array($user, $project));
    ...
    function getMyDoneProjectTasks($project, $limit = 5)
    {
       ...
       $user = (int) $user;
        ...
        $projectTasksStmt = $conn->prepare("SELECT tasks.*,tasks_assigned.user FROM tasks,tasks_assigned WHERE tasks.ID = tasks_assigned.task AND tasks_assigned.user = ? AND tasks.project = ? AND status=0 ORDER BY `end` ASC LIMIT $limit");
        $projectTasksStmt->execute(array($user, $project));
